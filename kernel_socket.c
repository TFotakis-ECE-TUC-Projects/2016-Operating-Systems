...
//	MSG("Socket write\n");
    ...
    SCB *scb = (SCB *) tmpScb;
    if (scb->socketType != PEER) {
//		MSG("Write from a non-peer socket\n");
        Mutex_Unlock(&kernel_mutex);
        return -1;
    }
    Mutex_Unlock(&kernel_mutex);
